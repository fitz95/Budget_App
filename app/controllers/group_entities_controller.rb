...
class GroupEntitiesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @group_entity.save
        format.html do
          redirect_to group_path(@group_entity.group_id), notice: 'Group entity was successfully created.'
        end
  ...
  def group_entity_params
    params.require(:group_entity).permit(:group_id, entity_attributes: %i[id name amount]).tap do |whitelisted|
